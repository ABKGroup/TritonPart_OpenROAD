...
benchmark_dir = "/home/fetzfs_projects/TritonPart/zhiang/TritonPart_OpenROAD/src/par/tritonpart_test/"
        ...
        for design in design_list:
            ...
            for seed in seed_list:
                ...
                for partitioner in partitioner_list:
                    ...
                    RunPartitioner(partitioner, partitioner_exe[partitioner], hypergraph_file, Nparts, UBfactor, seed)
                    end_time = time.time()
                    runtime = round(end_time - start_time, 3)
                    original_solution_file = hypergraph_file + ".part." + str(Nparts)
                    cutsize, balance = hMetisEvaluator(hypergraph_file, original_solution_file, Nparts, UBfactor)
                    print("cutsize : ", cutsize)
                    print("balance : ", balance)
                    cutsize_list.append(cutsize)
                    line += str(runtime) + "," + str(cutsize) + ","
                    balance_str = "["
                    if(os.path.exists(original_solution_file)):
